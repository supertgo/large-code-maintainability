from models import EXCLUDE_PATTERNS, File, DEFAULT_REPOSITORIES_DIR, DEFAULT_RESULTS_DIR
...
def extract_files(results_dir: Path, repositories_dir: Path):
    ...
    for result in results_dir.iterdir():
        ...
        java_files = sorted(
            repo_path.rglob("*.java"), key=lambda f: str(f.name).lower()
        )
        for java_file in java_files:
            if not any(part in str(java_file) for part in EXCLUDE_PATTERNS):
                file_json = File(
                    name=java_file.name,
